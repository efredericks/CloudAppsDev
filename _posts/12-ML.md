...
Warning: this section discusses [TensorFlow](https://www.tensorflow.org/) - an open-source library (in Python) for numerical computation (not strictly ML).  Figure X (c/o Google) shows how it works internally.  There is a lot to unpack here that we won't be going deeply into, however 
